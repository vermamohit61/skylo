...
namespace Drupal\switch_theme_based_page\Theme;
...
class PageThemeNegotiator implements ThemeNegotiatorInterface {
   ...
   * {@inheritdoc}
   */
  public function __construct(ConfigFactoryInterface $config_factory, CurrentPathStack $currentPath, PathMatcherInterface $pathMatcher, AccountProxyInterface $account, ModuleHandlerInterface $module_handler, RequestStack $request, DomainNegotiatorInterface $negotiator = NULL, LanguageManagerInterface $language_manager = NULL) {
  ...
  public function applies(RouteMatchInterface $route_match) {
    ...
    $spt_table = $this->configFactory->get('switch_theme_based_page.settings')->get('spt_table');
    ...
    foreach ($spt_table as $value) {
      ...
      if ($value['status'] == 1) {
        ...
        $condition = ($this->request->getCurrentRequest()->attributes->get("_route") != "system.403" && ($this->pathMatcher->matchPath($this->currentPath->getPath(), $value["pages"]) || $this->pathMatcher->matchPath(\Drupal::service('path_alias.manager')->getAliasByPath($this->currentPath->getPath()), $value["pages"])) && (!array_filter($value["roles"]) || !empty(array_intersect($value["roles"], $this->account->getRoles()))));
